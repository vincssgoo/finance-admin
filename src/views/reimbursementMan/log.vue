<template>
  <div class="app-container">
    <div style="margin-bottom:15px;">
      ...
      <el-date-picker v-model="listQuery.start_datetime"
        ...
        <div class="content_bottom">
          ...
          <div class="bottom"
            ...
            <div class="income_handle"
                 v-if="item.data_before.desc">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}说明：</span>
              <span class="span_width"
                    style="margin-left:20px;">{{item.data_before.desc}}</span>
              <span style="margin-left:30px;color:red;"
                    v-if="item.data_after.desc">变更后：</span>
              <span class="span_width"
                    style="margin-left:20px;color:red;"
                    v-if="item.data_after.desc">{{item.data_after.desc}}</span>
            </div>
            <div class="income_handle"
                 v-if="item.data_before.price">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}金额：</span>
              <span class="span_width"
                    style="margin-left:20px;">{{item.data_before.price}}</span>
              <span style="margin-left:30px;color:red;"
                    v-if="item.data_after.price">变更后：</span>
              <span class="span_width"
                    style="margin-left:20px;color:red;"
                    v-if="item.data_after.price">{{item.data_after.price}}</span>
            </div>
            <div class="income_handle"
                 v-if="item.data_before.pay_datetime || item.data_before.income_datetime">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}时间：</span>
              <span class="span_width"
                    style="margin-left:20px;">{{item.data_before.pay_datetime || item.data_before.income_datetime}}</span>
              <span style="margin-left:30px;color:red;"
            ...
            <div class="income_handle"
              ...
              <span class="span_width"
                    style="margin-left:20px;">{{item.data_before.type.title}}</span>
              <span style="margin-left:30px;color:red;"
                    v-if="item.data_after.type">变更后：</span>
              <span class="span_width"
                    style="margin-left:20px;color:red;"
                    v-if="item.data_after.type">{{item.data_after.type.title}}</span>
            </div>
            <div class="income_handle"
                 v-if="item.data_before.create_user">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}经手人：</span>
              <span class="span_width"
                    style="margin-left:20px;">{{item.data_before.create_user}}</span>
              <span style="margin-left:30px;color:red;"
                    v-if="item.data_after.create_user">变更后：</span>
              <span class="span_width"
                    style="margin-left:20px;color:red;"
                    v-if="item.data_after.create_user">{{item.data_after.create_user}}</span>
            </div>
            <div class="income_handle"
            ...
            <div class="income_handle"
              ...
              <span style="color:red;margin-right:50px;"
                    v-if="item.data_after.proof">修改收入凭证</span>
              <span style="color:red;margin-right:50px;"
                    v-if="item.data_after.pay_proof">修改支出凭证</span>
              <span style="color:red;"
                    v-if="item.data_after.expense_proof">修改报销凭证</span>
            </div>
          </div>
          <div class="bottom"
               v-else>
            <div class="income_handle"
                 v-if="item.data_after.desc">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}说明：</span>
              <span style="margin-left:50px;">{{item.data_after.desc}}</span>
            </div>
            <div class="income_handle"
                 v-if="item.data_after.price">
              <span>{{item.group == 1 ? '收入' : item.group == 2 ? '支出' : '报销'}}金额：</span>
